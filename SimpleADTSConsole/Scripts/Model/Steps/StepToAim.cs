...
{
    internal class StepToAim:Scheduler.IStep
    {
        ...
        public StepToAim(string name, IADTSTransportModel adts, PeriodDescriptor period, double aim, double rate, int callAfter = 1)
        {
            Name = name;
            _adts = adts;
            _period = period;
        ...
        {
            _adts.SendReceive(string.Format(unitCommandFormat, unit));
            if (cancel.WaitHandle.WaitOne(_period.Period))
                return false;

            var aim = _aim.ToString("F0");
            _adts.SendReceive(string.Format("SOUR:RATE {0},{1}", parameter, _rate.ToString("F0")));
            if (cancel.WaitHandle.WaitOne(_period.Period))
                return false;

            _adts.SendReceive(string.Format(aimCommandFormat, parameter, aim));
            if (cancel.WaitHandle.WaitOne(_period.Period))
                return false;

            _adts.SendReceive("STAT:OPER:EVEN?");
        ...
        public bool IsEnd(CancellationToken cancel)
        {
            ...
            string answer;
            _adts.SendReceive("STAT:OPER:EVEN?", out answer);
            if (cancel.WaitHandle.WaitOne(_period.Period))
                return false;
