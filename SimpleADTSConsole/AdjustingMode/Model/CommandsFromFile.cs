...
namespace SimpleADTSConsole.Tools
{
    public class CommandsFromFile
    {
        ...
        public IEnumerable<CommandAction> Parce(string file)
        {
            ...
            using (var stream = new MultiFilesStream(files))
            {
        ...
        {
            return line.Remove(0, _answerPrefix).Trim();
        }

        private string GetCommand(string line, LogVersion version)
        {
            var data = line.Remove(0, _commandPrefix).Trim();
            if (version == LogVersion.v2)
            {
                var index = data.IndexOf("[FromLast:", StringComparison.Ordinal);
                if (index >= 0)
                {
        ...
        public class MultiFilesStream:IDisposable
        {
            private IEnumerator<string> _files = null;
            private StreamReader _curFile = null;

            public MultiFilesStream(IEnumerable<string> files)
            {
